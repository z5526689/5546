...
# ----------------------------------------------------------------------------
ROOTDIR = os.path.join('toolkit', 'project1')
...
def line_to_dict(line):
    ...
    line_dict = {}
    colums = COLUMNS
    colums_width = COLWIDTHS
    start = 0
    for colum in colums:
        end = start + colums_width[colum]
        colum_data = line[start:end].strip()
        line_dict[colum] = colum_data
        start = end
    return  line_dict
    ...
    if tickers_lst != None:
        if not tickers_lst:
            ...
            for ticker in tickers_lst:
                if ticker.lower() not in tickers_lst:
    ...
    for ticker in tickers_lst:
        ...
        for line in lines:
            ...
            if col_lst is not None:
                line_dict = {col: line_dict[col] for col in col_lst}
